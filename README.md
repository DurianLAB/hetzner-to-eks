...
flowchart TD
    ...
    subgraph STAGE_1["**STAGE 1: Hetzner Foundation**"]
        ...
        DEPLOY_HETZNER --> K8S_HETZNER["Self-managed Kubernetes (RKE2 / Talos / K3s)"]
    ...
    subgraph STAGE_2["**STAGE 2: Hetzner + AWS Hybrid**"]
        ...
        STABLE -->|Yes| HETZNER_STAYS["Keep Hetzner for: Dev/Staging, Non-critical workloads, EU-only services"]
        ...
        subgraph ST1_HETZNER["Hetzner Cloud (Nuremberg / Finland)"]
            ...
            subgraph ST1_K8S["Kubernetes Cluster (Self-Managed)"]
                subgraph ST1_NS1["Namespace: production"]
                    ING1["Ingress Controller (nginx / Traefik)"]
                    API1["API Service"]
                    WEB1["Web App"]
                    WORKER1["Background Workers"]
                    DB1["PostgreSQL (Self-managed)"]
                    CACHE1["Redis (Self-managed)"]
                    OBJ1["Object Storage (Hetzner Storage Box)"]
                end
            ...
            subgraph ST1_MGMT["Management"]
                ...
                LOGS1["Logging (Loki)"]
                CI1["CI/CD Runner (GitHub Actions)"]
            end
        end
    ...
    subgraph STAGE2["**STAGE 2: Hybrid Architecture (Hetzner + AWS EKS)**"]
        ...
        subgraph AWS["AWS (US / Regulated / Critical)"]
            ...
            subgraph AWS_DATA["Managed Data Services"]
                AWS_RDS["RDS PostgreSQL (Multi-AZ)"]
    ...
    subgraph STAGE3["**STAGE 3: Full AWS EKS Architecture**"]
        ...
        subgraph AWS_VPC["AWS VPC"]
            ...
            subgraph DATA_LAYER["Data Services (AWS Managed)"]
                ...
                RDS_PRIMARY["RDS PostgreSQL (Multi-AZ)"]
...
flowchart TD
    ...
    subgraph COMPLIANCE["Compliance Triggers"]
        Q1{Do US enterprise customers require SOC 2?}
        Q1 -->|Yes| COMPLY_SOC2["SOC 2 Required - Migrate to EKS"]
        Q1 -->|No| Q2

        Q2{Do you process payment card data?}
        Q2 -->|Yes| COMPLY_PCI["PCI DSS Required - AWS is Level 1 Service Provider"]
        Q2 -->|No| Q3

        Q3{Do you handle PHI / healthcare data?}
        Q3 -->|Yes| COMPLY_HIPAA["HIPAA Required - Migrate with BAA signed"]
        Q3 -->|No| Q4

        Q4{Do EU regulations require BSI C5 / KRITIS?}
        Q4 -->|Yes| COMPLY_BSI["Hetzner qualifies (BSI C5)"]
        Q4 -->|No| Q5
    end

    subgraph COST["Cost Triggers"]
        Q5{Monthly infra spend > $5,000?}
        Q5 -->|Yes| COST_ANALYSIS["Analyze Reserved / Savings Plans"]
        Q5 -->|No| Q6

        Q6{Need managed services (RDS, ElastiCache)?}
        Q6 -->|Yes| COST_MANAGED["Hetzner lacks managed services"]
        Q6 -->|No| Q7
    end

    subgraph SCALE["Scale & Reliability Triggers"]
        Q7{Need multi-region HA with < 1hr RTO?}
        Q7 -->|Yes| SCALE_HA["Hetzner has no formal SLA"]
        Q7 -->|No| Q8

        Q8{Experiencing Hetzner outage impact?}
        Q8 -->|Yes| SCALE_OUTAGE["No redundancy on Hetzner"]
        Q8 -->|No| Q9

        Q9{Need < 10ms latency globally?}
        Q9 -->|Yes| SCALE_GLOBAL["Hetzner limited regions"]
        Q9 -->|No| Q10
    end

    subgraph SECURITY["Security Triggers"]
        Q10{Need customer-facing pentest reports?}
        Q10 -->|Yes| SEC_PENTEST["AWS Artifact provides audit reports"]
        Q10 -->|No| Q11

        Q11{Team lacks Kubernetes ops expertise?}
        Q11 -->|Yes| SEC_OPS["Self-managed K8s risk"]
        Q11 -->|No| Q12
    end

    subgraph DECISION["Decision Matrix"]
        Q12{None of the above?}
        Q12 -->|Stay on Hetzner| DECISION_STAY["STAY ON HETZNER - Re-assess quarterly"]
        Q12 -->|Any triggers| DECISION_MIGRATE["MIGRATE TO HYBRID - Stage 2"]
        Q12 -->|Multiple critical| DECISION_FULL["MIGRATE FULL TO EKS - Stage 3"]
    end

    COMPLY_SOC2 -.-> DECISION_MIGRATE
    COMPLY_PCI -.-> DECISION_MIGRATE
    COMPLY_HIPAA -.-> DECISION_MIGRATE
    COMPLY_BSI -.-> DECISION_STAY
    COST_ANALYSIS -.-> DECISION_MIGRATE
    COST_MANAGED -.-> DECISION_MIGRATE
    SCALE_HA -.-> DECISION_MIGRATE
    SCALE_OUTAGE -.-> DECISION_MIGRATE
    SCALE_GLOBAL -.-> DECISION_MIGRATE
    SEC_PENTEST -.-> DECISION_MIGRATE
    SEC_OPS -.-> DECISION_MIGRATE
```
...
flowchart LR
    ...
    subgraph OUTPUTS["Recommended Stage"]
        ...
        R2["Stage 2: Hetzner + EKS Hybrid US + EU mixed Some regulated workloads Growing startup"]
        R3["Stage 3: Full AWS EKS US enterprise primary PCI DSS / HIPAA Series B+ / IPO path"]
    end
...
```mermaid
gantt
    title Startup Infrastructure Journey: Hetzner to AWS EKS
    dateFormat  YYYY-MM
    axisFormat  %m/%Y
...
sequenceDiagram
    ...
    Note over GH,DEV: Full audit trail: CloudTrail logs every API call
```
